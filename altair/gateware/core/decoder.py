...
class DecoderUnit(Elaboratable):
    ...
    def elaborate(self, platform: Platform) -> Module:
        ...
        def match(op, f3=None, f7=None, f12=None):
            op_match  = opcode == op
        ...
        with m.If(self.enable):
            ...
            m.d.sync += [
                ...
                self.is_imm.eq(match(Opcode.OP_IMM)),
            ]
            ...
            m.d.sync += [
                ...
                self.inst_beq.eq(match(Opcode.BRANCH, Funct3.BEQ)),
                self.inst_bne.eq(match(Opcode.BRANCH, Funct3.BNE)),
                self.inst_blt.eq(match(Opcode.BRANCH, Funct3.BLT)),
                self.inst_bge.eq(match(Opcode.BRANCH, Funct3.BGE)),
                self.inst_bltu.eq(match(Opcode.BRANCH, Funct3.BLTU)),
                self.inst_bgeu.eq(match(Opcode.BRANCH, Funct3.BGEU)),
                self.inst_lb.eq(match(Opcode.LOAD, Funct3.B)),
                self.inst_lh.eq(match(Opcode.LOAD, Funct3.H)),
                self.inst_lw.eq(match(Opcode.LOAD, Funct3.W)),
                self.inst_lbu.eq(match(Opcode.LOAD, Funct3.BU)),
                self.inst_lhu.eq(match(Opcode.LOAD, Funct3.HU)),
                self.inst_sb.eq(match(Opcode.STORE, Funct3.B)),
                self.inst_sh.eq(match(Opcode.STORE, Funct3.H)),
                self.inst_sw.eq(match(Opcode.STORE, Funct3.W)),
                self.inst_addi.eq(match(Opcode.OP_IMM, Funct3.ADD)),
                self.inst_slti.eq(match(Opcode.OP_IMM, Funct3.SLT)),
                self.inst_sltiu.eq(match(Opcode.OP_IMM, Funct3.SLTU)),
                self.inst_xori.eq(match(Opcode.OP_IMM, Funct3.XOR)),
                self.inst_ori.eq(match(Opcode.OP_IMM, Funct3.OR)),
                self.inst_andi.eq(match(Opcode.OP_IMM, Funct3.AND)),
                self.inst_slli.eq(match(Opcode.OP_IMM, Funct3.SLL, 0)),
                self.inst_srli.eq(match(Opcode.OP_IMM, Funct3.SR, Funct7.SRL)),
                self.inst_srai.eq(match(Opcode.OP_IMM, Funct3.SR, Funct7.SRA)),
                self.inst_add.eq(match(Opcode.OP, Funct3.ADD, Funct7.ADD)),
                self.inst_sub.eq(match(Opcode.OP, Funct3.ADD, Funct7.SUB)),
                self.inst_sll.eq(match(Opcode.OP, Funct3.SLL, 0)),
                self.inst_slt.eq(match(Opcode.OP, Funct3.SLT, 0)),
                self.inst_sltu.eq(match(Opcode.OP, Funct3.SLTU, 0)),
                self.inst_xor.eq(match(Opcode.OP, Funct3.XOR, 0)),
                self.inst_srl.eq(match(Opcode.OP, Funct3.SR, Funct7.SRL)),
                self.inst_sra.eq(match(Opcode.OP, Funct3.SR, Funct7.SRA)),
                self.inst_or.eq(match(Opcode.OP, Funct3.OR, 0)),
                self.inst_and.eq(match(Opcode.OP, Funct3.AND, 0)),
                self.inst_fence.eq(match(Opcode.FENCE, Funct3.FENCE)),
                self.inst_fencei.eq(match(Opcode.FENCE, Funct3.FENCEI)),
                self.inst_csrrw.eq(match(Opcode.SYSTEM, Funct3.CSRRW)),
                self.inst_csrrs.eq(match(Opcode.SYSTEM, Funct3.CSRRS)),
                self.inst_csrrc.eq(match(Opcode.SYSTEM, Funct3.CSRRC)),
                self.inst_csrrwi.eq(match(Opcode.SYSTEM, Funct3.CSRRWI)),
                self.inst_csrrsi.eq(match(Opcode.SYSTEM, Funct3.CSRRSI)),
                self.inst_csrrci.eq(match(Opcode.SYSTEM, Funct3.CSRRCI)),
                self.inst_xcall.eq(match(Opcode.SYSTEM, Funct3.PRIV, None, Funct12.ECALL)),
                self.inst_xbreak.eq(match(Opcode.SYSTEM, Funct3.PRIV, None, Funct12.EBREAK)),
                self.inst_mret.eq((self.privmode == PrivMode.Machine) & match(Opcode.SYSTEM, Funct3.PRIV, None, Funct12.MRET)),
                self.inst_wfi.eq(match(Opcode.SYSTEM, Funct3.PRIV, None, Funct12.WFI))
            ]
            if self.enable_rv32m:
                m.d.sync += [
                    self.inst_mul.eq(match(Opcode.OP, Funct3.MUL, Funct7.MULDIV)),
                    self.inst_mulh.eq(match(Opcode.OP, Funct3.MULH, Funct7.MULDIV)),
                    self.inst_mulhsu.eq(match(Opcode.OP, Funct3.MULHSU, Funct7.MULDIV)),
                    self.inst_mulhu.eq(match(Opcode.OP, Funct3.MULHU, Funct7.MULDIV)),
                    self.inst_div.eq(match(Opcode.OP, Funct3.DIV, Funct7.MULDIV)),
                    self.inst_divu.eq(match(Opcode.OP, Funct3.DIVU, Funct7.MULDIV)),
                    self.inst_rem.eq(match(Opcode.OP, Funct3.REM, Funct7.MULDIV)),
                    self.inst_remu.eq(match(Opcode.OP, Funct3.REMU, Funct7.MULDIV))
                ]
